...
#Despliega las cabeceras o nombres de las columnas, las cuales corresponden a las variables.
names(sarco)                     
```
```{r}
...
```
```{r}
#Ofrece una previsualización del tipo de dato de cada variable y su contenido, así como  el número de registros.
str(sarco)
```
```{r}
...
```
```{r}
#Sentencia que limpia las filas que tienen valores vacios, y guarda los nuevos registros en la variable sarco1
...
```{r}
#Sentencia SQL que permite mantener registros únicos por paciente. Es decir, elimina los valores duplicados como se visualizaba en la consulta anterior para sarco1.
sarco2 <- sqldf('SELECT DISTINCT * FROM sarco1')
#Confirma el nuevo número de registros, almacenados en sarco2, luego de eliminar valores duplicados en el conjunto de datos sarco1.
nrow(sarco2)
```
```{r}
...
```
```{r}
#Crea el factor de Sympton para hacer el group_by. Es decir, se modifica la naturaleza de la variable Sympton con fines de hacer el conteo
...
```{r}
#Realiza una ordenación del conjunto de datos, almacenándolo en la variable sarco2.sorted
sarco2.sorted <- sarco2[order(sarco2$Symptom),]
library(plyr)

#La función ddply() recibe el data frame, lo recorta en fragmentos basados en uno o más factores, para luego traer los resultados como un nuevo conjunto de datos llamado itemList.
...
knitr::include_graphics("D:/Downloads/Ciencia_de_datos_aplicada_a_la_salud/Salidas/punto5.jpg", auto_pdf = getOption("knitr.graphics.auto_pdf",FALSE))
```
...
```{r pressure, echo=FALSE, fig.cap="", out.width = '50%'}
knitr::include_graphics("D:/Downloads/Ciencia_de_datos_aplicada_a_la_salud/Salidas/punto6.jpg", auto_pdf = getOption("knitr.graphics.auto_pdf",FALSE))
```
